...
def create_conv_model(fingerprint_input, model_settings, is_training):
    ...
    final_fc_weights = tf.Variable(
        tf.truncated_normal([second_conv_element_count, label_count], stddev=0.01)
    )
...
def create_low_latency_conv_model(fingerprint_input, model_settings, is_training):
    ...
    label_count = model_settings["label_count"]
    final_fc_weights = tf.Variable(
        tf.truncated_normal([second_fc_output_channels, label_count], stddev=0.01)
    )
    final_fc_bias = tf.Variable(tf.zeros([label_count]))
...
def create_low_latency_svdf_model(
    ...
    if input_shape[-1].value % input_frequency_size != 0:
        raise ValueError(
            "Inputs feature dimension %d must be a multiple of " "frame size %d",
